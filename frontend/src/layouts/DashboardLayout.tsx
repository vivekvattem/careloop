  ...
  return (
    <div className="min-h-screen bg-mist">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-slate-200 bg-white p-5 lg:flex lg:flex-col">
        <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-care-600 font-bold text-white shadow-sm">C</span><div><p className="font-bold text-ink">CareLoop</p><p className="text-xs text-slate-500">Connected care</p></div></div>
      ...
      <div className="lg:pl-64"><header className="sticky top-0 z-10 border-b border-slate-200/80 bg-white/95 backdrop-blur"><div className="flex min-h-16 items-center justify-between px-4 sm:px-7"><div className="flex items-center gap-3 lg:hidden"><span className="grid h-9 w-9 place-items-center rounded-xl bg-care-600 font-bold text-white">C</span><span className="font-bold text-ink">CareLoop</span></div><p className="hidden text-sm font-medium capitalize text-slate-500 lg:block">{user?.role} workspace</p><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-semibold text-ink">{user?.full_name}</p><p className="text-xs capitalize text-slate-500">{user?.role}</p></div><span className="grid h-9 w-9 place-items-center rounded-full bg-care-100 text-sm font-bold text-care-900">{user?.full_name?.slice(0, 1).toUpperCase()}</span><button className="ui-button-outline min-h-9 px-3 py-1.5" type="button" onClick={handleLogout}>Log out</button></div></div></header>
