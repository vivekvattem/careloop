...
const emptyMedication = (): PrescriptionItemInput => ({ medication_name: "", dosage: "", route: null, frequency_per_day: 1, reminder_times: [], start_date: new Date().toISOString().slice(0, 10), end_date: null, food_instructions: null, additional_instructions: null });
const emptyVisit = (): CompleteVisitInput => ({ clinical_note: { original_notes: "", diagnosis: null, follow_up_instructions: "", recommended_follow_up_date: null }, prescription: { general_instructions: null, items: [] } });
...
export function DoctorDashboardPage() {
  ...
  const loadIntelligence = async (id: string) => {
    ...
    if (detail.status === "completed") {
      ...
      setVisit({ clinical_note: { original_notes: record.original_notes, diagnosis: record.diagnosis, follow_up_instructions: record.follow_up_instructions, recommended_follow_up_date: record.recommended_follow_up_date }, prescription: { general_instructions: record.general_instructions, items: record.items } });
